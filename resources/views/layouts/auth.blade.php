        ...
        <nav class="menu" style="background-color: #43ced3">
            ...
            <li>
              <div  ><a href="{{ route('logout') }}"
                       onclick="event.preventDefault(); document.getElementById('logout-form-icon').submit();">
              ...
              {{-- The icon above was a plain GET link to /logout. That route was
                   admin-only, so non-admins were never logged out; it is gone and
                   logout is a POST now. The page's other logout form sits inside a
                   commented-out block, hence this dedicated one. --}}
              <form id="logout-form-icon" action="{{ route('logout') }}" method="POST" class="d-none">
                @csrf
              </form>
